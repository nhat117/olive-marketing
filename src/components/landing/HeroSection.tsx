    ...
    <section className="relative min-h-[100svh] overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-[1920px] flex-col justify-center px-5 pb-36 pt-[4.75rem] max-md:items-center max-md:text-center md:items-stretch md:px-10 md:pb-20 md:pt-[5.25rem] md:text-left lg:px-12 lg:pb-20">
        <div className="max-w-2xl w-full max-md:mx-auto">
          <p
            className={`hero-anim hero-anim-d1 ${m3OverlineAccent} mb-4 max-md:mx-auto md:mb-5 [text-shadow:0_1px_2px_var(--color-background),0_0_18px_color-mix(in_srgb,var(--color-background)_60%,transparent)] md:[text-shadow:none]`}
          >
            {t("overline")}
          </p>
          <h1 className="hero-anim hero-anim-d2 mb-5 text-balance font-headline text-[2.5rem] font-normal italic leading-[1.06] tracking-tight text-primary max-md:mx-auto md:mb-8 md:text-[4rem] lg:text-[clamp(4.5rem,8vw,5.75rem)] [text-shadow:0_2px_4px_var(--color-background),0_0_32px_color-mix(in_srgb,var(--color-background)_65%,transparent)] md:[text-shadow:0_1px_2px_color-mix(in_srgb,var(--color-background)_35%,transparent)]">
            {t("headline")}
          </h1>
      ...
      <div
        className={`hero-anim hero-anim-d5 absolute bottom-6 left-1/2 z-20 w-[min(100%-2.5rem,22rem)] max-w-[min(100%-2.5rem,22rem)] -translate-x-1/2 px-5 py-4 md:bottom-8 md:left-auto md:right-10 md:max-w-sm md:translate-x-0 md:px-6 md:py-5 ${m3GlassExpressive} ${m3TransitionExpressive}`}
      >
